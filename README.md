...
- [Backup and reinstall guide](#legacy-backup-and-reinstall)

Once you are on the new layout, shipped examples live in `/helpers/examples/` and editable global shortcuts stay in `/shortcuts/`.
...
- Shipped examples now live under `helpers/examples/`.
...
If the plugin feels empty on first install, copy a starter file from `/a0/usr/plugins/slash_shortcuts/helpers/examples/` into the active `shortcuts/` scope, or create a new shortcut directly from the manager.
...
- shipped examples live in `/a0/usr/plugins/slash_shortcuts/helpers/examples/`
...
The repo ships example shortcuts in `/a0/usr/plugins/slash_shortcuts/helpers/examples/` such as:
- `summarize`
- `fix`
- `review`
- `tone`
